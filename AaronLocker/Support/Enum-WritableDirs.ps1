...
If set, output includes the names of the non-admin entities that have write
permissions
...
.\Enum-WritableDirs.ps1 C:\Windows\System32
...
.EXAMPLE
.\Enum-WritableDirs.ps1 C:\Windows\System32 -ShowGrantees
...
.EXAMPLE
$x = [xml](.\Enum-WritableDirs.ps1 C:\Windows\System32 -ShowGrantees -OutputXML)
...
param(
	...
	# If -ShowGrantees on the command line, shows which non-admin accounts
	# are granted access
	[switch]
	$ShowGrantees = $false,

	# If -DontFilterNTService set, does not filter out access granted to
	# NT SERVICE\ accounts (other than TrustedInstaller)
	[switch]
	$DontFilterNTService = $false,

	# If -OutputXML, output is XML
	[switch]
	$OutputXML = $false,
